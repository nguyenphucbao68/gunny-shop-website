...
$(document).ready(function () {
    ...
    for (var i = 0; i < canvas.length; i++) {
        ctx[canvas[i]] = ctxDoc[canvas[i]] = document.getElementById(canvas[i])
        var canvasWidth = ctx[canvas[i]].width
        var canvasHeight = ctx[canvas[i]].height
        ctx[canvas[i]] = ctx[canvas[i]].getContext('2d')
    }
    ...
    $('#tabs-2 .btn-color-make').click(function () {
        ...
        $('.avatar-view .frame.active:not(#wing)').has('.animate-border-blink').each(function () {
            ...
            overlay.onload = function () {
                if (colorSkin != 0) {
                    if (cat == 'face') {
                        drawEMatAnimation(ctx[cat], overlay, item_equip[cat], hex, cat)
                    } else if (cat != 'suits') {
                        draw(ctx[cat], overlay, item_equip[cat], hex, cat)
                    }
                } else {
                    if (cat == 'face') {
                        drawEMatAnimation(ctx[cat], overlay, item_equip[cat], hex)
                    } else if (cat != 'suits') {
                        draw(ctx[cat], overlay, item_equip[cat], hex)
                    }
                }
            }
        })
        ...
        if (e != 0) {
            ...
            if (circleSetting != 0) {
                if (!resSuit['circle']) {
                    resSuit = {... {
                            'circle': {
                                'original': domain + '/equip/circlelight/' + circleSetting + '.png'
                            }
                        }, ...resSuit
                    }
                } else {
    ...
    function updateListItemInCart(setHtmlList = 0, type = 1, data = {}) {
        if (type == 1) {
            // xử lý trường hợp 1
            for (var key in suit)
                if (suit[key] != 0) item_bag[key] = suit[key]
    ...
    function load_wings() {
        ...
        readSprite.success(function (xml) {
            ...
            wingsImage = sprite
            drawClass.start(ctx['wing'], sprite)
        })
    }
    ...
    function drawEMatSkinAnimation(context, original, skin, hex = '#e7d013', key = '') {
        var countSkin = 0,
            eCloseSkin = 0
        ...
        matSkin = setInterval(function () {
            ...
            if (countSkin == 1 || countSkin == 5) {
                drawEmatSkinOnces(context, original, skin, hex, 0, 0, '0', key)
            } else if (countSkin == 2 || countSkin == 4) {
                drawEmatSkinOnces(context, original, skin, hex, 750, 0, '1', key)
            } else if (countSkin == 3) {
                drawEmatSkinOnces(context, original, skin, hex, 500, 0, '2', key)
                eCloseSkin++
            } else if (countSkin == 6) {
                if (eCloseSkin == 2) {
                    setTimeout(function () {
    ...
    function drawEmatSkinOnces(context, original, skin, hex = '#e7d013', x, y, count, key = '') {
        context.clearRect(0, 0, canvasWidth, canvasHeight)
        ...
        if (!(count in screenshotSkin[key])) {
            imageDoc = new Image()
            imageDoc.src = ctxDoc[key].toDataURL()
    ...
    function drawSkinColor(context, original, skin, hex = '#e7d013', key = '') {
        context.clearRect(0, 0, canvasWidth, canvasHeight)
    ...
    function draw(context, overlay, original, hex = '#e7d013', key = '') {
        context.clearRect(0, 0, canvasWidth, canvasHeight)
    ...
    function drawEMatAnimation(context, overlay, original, hex, key = '') {
        var count = 0,
            eClose = 0
        ...
        test = setInterval(function () {
            count++
            if (count == 1 || count == 5) {
                drawEmatOnces(context, overlay, original, hex, 0, 0, '0', key)
            } else if (count == 2 || count == 4) {
                drawEmatOnces(context, overlay, original, hex, 750, 0, '1', key)
            } else if (count == 3) {
                drawEmatOnces(context, overlay, original, hex, 500, 0, '2', key)
                eClose++
            } else if (count == 6) {
                if (eClose == 2) {
                    setTimeout(function () {
    ...
    function drawEmatOnces(context, overlay, original, hex = '#e7d013', x, y, count, key = '') {
        context.clearRect(0, 0, canvasWidth, canvasHeight)
    ...
    function drawface(img, width, height) {
        ...
        eff_run = setInterval(function () {
            count++
            if (count == 1 || count == 5) {
                ctx['face'].clearRect(0, 0, canvasWidth, canvasHeight)
                    // mở mắt hoàn toàn
                ctx['face'].drawImage(img, 0, 0, 250, 312, posEquip['face']['x'], posEquip['face']['y'], width, height)
            } else if (count == 2 || count == 4) {
                ctx['face'].clearRect(0, 0, canvasWidth, canvasHeight)
                    // mở hoặc đóng nửa (1/2) mắt
                ctx['face'].drawImage(img, 750, 0, 250, 312, posEquip['face']['x'], posEquip['face']['y'], width, height)
            } else if (count == 3) {
                ctx['face'].clearRect(0, 0, canvasWidth, canvasHeight)
                    // nhắm mắt, tăng biến eClose để timeOut bên dưới chuẩn bị cho việc mở mắt
                ctx['face'].drawImage(img, 500, 0, 250, 312, posEquip['face']['x'], posEquip['face']['y'], width, height)
                eClose++
            } else if (count == 6) {
                if (eClose == 2) {
