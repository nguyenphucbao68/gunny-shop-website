(function ($) {
	const configURL = "http://localhost/library/config.php";
	...
	const gnLib = {
		...
		htmlHandle($this, code, init, plugin, methodSettings, state = true, attrData = "gnData") {
			...
			html.css("display", gnShopHtmlStore.getDisplay(code));
			$this.html("");
			init.data.map(item => {
				var frame = html.clone().appendTo($this.selector).removeAttr("gnpluginhtml");
				if (frame.attr(attrData)) gnLib.readDataHtml(frame, $this, item, init, plugin, methodSettings, attrData).removeAttr(attrData);
				frame
					.find(`[${attrData}]`)
					.each(function () {
	...
	// gnLib.handleObject({
	// 	addProperty: ["img", "nameSex", "nameQuality", "type","cat"],
	// 	limit: 8,
	// 	filter: {"NeedSex": 1, "category_id": 4},
	// 	page: 2
	// })
	// $("[gnPlugin='gnSearchDW']").gnSetupHtmlAjax({
	// 	html: "gnSearchDW",
	// 	load: {
	...
	// $.gnSetupAPI("listitem", {limit: 5000}, function(data) {
	// 	$("[gnPluginHtml='gnHotItem']").gnHtmlHandle("load", {
	// 		data
	// 	});
	// })
